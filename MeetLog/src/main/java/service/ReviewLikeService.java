...
public class ReviewLikeService {
    ...
    /**
     * 좋아요를 토글하고, 최신 상태와 개수를 Map으로 반환하는 단일 메서드
     * @return Map<String, Object> - "isLiked" (boolean), "newLikeCount" (int) 포함
     */
    public Map<String, Object> toggleLike(int userId, int reviewId) throws Exception {
        try (SqlSession session = MyBatisSqlSessionFactory.getSqlSession(false)) { // 트랜잭션 수동 관리를 위해 false 전달
            try {
                ...
                // 2. 현재 좋아요를 눌렀는지 확인
                boolean isAlreadyLiked = reviewLikeDAO.isLikedByUser(session, params) > 0;
                
                // 3. 좋아요/취소 DB 작업 수행
                if (isAlreadyLiked) {
                    reviewLikeDAO.deleteLike(session, params); // 좋아요 취소
                    reviewDAO.decrementLikes(session, reviewId); // likes 카운트 감소
                } else {
                    reviewLikeDAO.insertLike(session, params); // 좋아요 추가
                    reviewDAO.incrementLikes(session, reviewId); // likes 카운트 증가
                }

                // 4. 최신 좋아요 수 다시 조회
                Review updatedReview = reviewDAO.findByIdWithLikes(session, reviewId);
                ...
                result.put("newLikeCount", updatedReview.getLikes());
                return result;

            } catch (Exception e) {
                session.rollback(); // 에러 발생 시 롤백
                e.printStackTrace();
                throw new Exception("좋아요 처리 중 오류가 발생했습니다.", e);
            }
        }
    }
    ...
    public List<User> getUsersWhoLikedReview(int reviewId) {
        List<User> likers = reviewLikeDAO.findUsersWhoLikedReview(reviewId);
        if (likers == null) {
