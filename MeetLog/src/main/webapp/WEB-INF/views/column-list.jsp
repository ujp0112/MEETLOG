...
@
layer utilities { .line-clamp-2 { display:-webkit-box;
	-webkit-line-clamp: 2;
		...
		<main class="flex-grow">
			<div class="container mx-auto p-4 md:p-8">
				<div class="mb-8 flex justify-between items-center">
					...
					<div>
						<h2 class="text-2xl md:text-3xl font-bold text-slate-800 mb-2">맛집
							칼럼</h2>
				...
				<c:choose>
					...
					<c:when test="${not empty columns}">
						<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
							<c:forEach var="column" items="${columns}">
								...
								<div
									class="bg-white rounded-xl shadow-lg overflow-hidden group transform hover:-translate-y-1 transition-all duration-300">
									<a
										...
										<div class="overflow-hidden">
											<mytag:image fileName="${column.image}"
												...
												cssClass="w-full h-48 object-cover group-hover:scale-105 transition-transform duration-300" />
										</div>
									...
									<div class="p-5">
										...
										<a
											href="${pageContext.request.contextPath}/column/detail?id=${column.id}">
											<h3
												class="text-lg font-bold text-slate-800 mb-2 line-clamp-2 h-14 group-hover:text-sky-600 transition-colors">${column.title}</h3>
										</a>


										<div
											class="flex items-center justify-between text-sm text-slate-500 mt-4 pt-4 border-t border-slate-100">
					...
					<c:otherwise>
						<div class="col-span-full text-center py-20">
							...
							<h3 class="text-xl font-bold text-slate-800 mb-2">아직 작성된 칼럼이
								없습니다.</h3>
