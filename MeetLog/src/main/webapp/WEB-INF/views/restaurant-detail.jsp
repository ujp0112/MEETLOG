                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            <div class="lg:col-span-2">
                                ...
                                <div class="custom-card p-4">
                                    <c:choose>
                                        <c:when test="${not empty restaurant.image and fn:startsWith(restaurant.image, 'http')}"><c:set var="imageUrl" value="${restaurant.image}" /></c:when>
                                        <c:when test="${not empty restaurant.image}"><c:set var="imageUrl" value="${pageContext.request.contextPath}/${restaurant.image}" /></c:when>
                                        <c:otherwise><c:set var="imageUrl" value="https://placehold.co/800x400/e0e0e0/ffffff?text=No+Image" /></c:otherwise>
                                    </c:choose>
                                    <div class="overflow-hidden rounded-xl shadow-md">
                                        <img src="${imageUrl}" alt="${restaurant.name}" class="w-full h-80 object-cover">
                                    </div>
                                </div>
                                            ...
                                            <c:forEach var="review" items="${reviews}">
                                                <div class="border-t border-gray-200 py-4">
                                                    <div class="flex justify-between items-center mb-2">
                                                        ...
                                                        <%-- ▼▼▼ 오류 수정 ▼▼▼ --%>
                                                        <%-- fmt:formatDate 대신 EL에서 직접 format 메소드 호출 --%>
