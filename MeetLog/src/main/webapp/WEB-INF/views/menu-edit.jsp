...
<head>
    ...
    <title>메뉴 수정 - ${restaurant.name}</title>
        ...
        <header class="bg-white shadow-sm border-b">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between items-center h-16">
                    ...
                    <h1 class="text-xl font-semibold text-gray-900">메뉴 수정</h1>
                </div>
            </div>
        </header>
        ...
        <main class="max-w-2xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
            <div class="bg-white shadow rounded-lg">
                <div class="px-6 py-4 border-b border-gray-200">
                    <h2 class="text-lg font-medium text-gray-900">${restaurant.name} - ${menu.name} 수정</h2>
                </div>
                
                <form action="/MeetLog/business/restaurants/${restaurant.id}/menus/edit/${menu.id}" method="POST" enctype="multipart/form-data" class="px-6 py-4">
                    <div class="space-y-6">
                        <!-- 메뉴명 -->
                        <div>
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="3"
                                      class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">${menu.description}</textarea>
                        </div>

                        <!-- 가격 -->
                        <div>
                            <label for="price" class="block text-sm font-medium text-gray-700">가격 (원) *</label>
                            <input type="text" id="price" name="price" value="${menu.price}" required
                                   class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                        </div>
                        ...
                        <c:if test="${not empty menu.image}">
                            <div>
                                <label class="block text-sm font-medium text-gray-700">현재 이미지</label>
                                <div class="mt-1">
                                    <img src="${menu.image}" alt="${menu.name}" class="h-32 w-32 object-cover rounded-lg">
                                </div>
                            </div>
                        </c:if>
                        ...
                        <div>
                            <label for="image" class="block text-sm font-medium text-gray-700">
                                ${not empty menu.image ? '이미지 변경' : '메뉴 이미지'}
                            </label>
                        ...
                        <div>
                            <div class="flex items-center">
                                <input type="checkbox" id="isActive" name="isActive" value="true" ${menu.isActive ? 'checked' : ''}
