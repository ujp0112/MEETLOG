...
<body>
						...
						<tbody>
							<c:forEach var="m" items="${materials}">
								<tr id="row-${m.id}" data-id="${m.id}"
									...
									data-unitprice="${m.unitPrice}" data-step="${m.step}"
									...
									<td>${m.name}</td>
									<td>${m.unit}</td>
									<td class="cell-id"><fmt:formatNumber
			...
			<div class="bd">
				...
				<form id="editForm" class="form" method="post"
					...
					<input type="hidden" name="id" id="f-id" /> <input type="hidden"
						name="prev_img_path" id="f-prev" />
					<div class="row">
						<label class="label">재료 사진</label>
						<div class="image-field">
							<label class="preview-circle" for="f-image" title="이미지 선택">
								<%-- id만 f-preview로 유지 --%> <img id="f-preview"
								src="${contextPath}/img/plus.png" alt="preview" />
							</label>
							<div>
					...
					<div class="row">
						<label class="label" for="f-name">재료명<span
							style="color: #ef4444"> *</span></label> <input class="input" type="text"
							name="name" id="f-name" required />
					</div>
					<div class="row">
						<label class="label" for="f-unit">단위<span
							style="color: #ef4444"> *</span></label> <input class="input" type="text"
							name="unit" id="f-unit" placeholder="kg / L / 개" required />
					</div>
					<div class="row">
						<label class="label" for="f-unitPrice">단가</label> <input
							class="input" type="number" name="unit_price" id="f-unitPrice"
							step="0.01" min="0" />
					</div>
					<div class="row">
						<label class="label" for="f-step">주문단위(팩당 단위수)</label> <input
							class="input" type="number" name="step" id="f-step" step="0.01"
							min="0" />
					</div>

					<!-- 이미지 URL(텍스트 우선) + 파일 업로드(대체) -->
					<!-- <div class="row">
						<label class="label" for="f-imgPath">이미지 URL</label> <input
							class="input" type="text" name="img_path" id="f-imgPath"
							placeholder="/uploads/materials/xxx.jpg" />
					</div> -->
		...
		<div class="dialog">
			...
			<div class="bd">
				...
				<form id="appendForm" class="form"
					...
					<div class="row">
						...
						<div class="image-field">
							<label class="preview-circle" for="a-image" title="이미지 선택">
								<img id="a-preview" src="${contextPath}/img/plus.png"
								alt="preview" />
							</label>
							<div>
								<input class="input" type="text" id="a-selectFile"
									placeholder="선택된 파일 없음" readonly />
								<div class="hint">정사각형 이미지를 권장합니다. (JPG/PNG)</div>
							</div>
						</div>
					...
					<div class="row">
						<label class="label" for="a-name">재료명<span
							style="color: #ef4444"> *</span></label> <input class="input" type="text"
							name="name" id="a-name" required />
					</div>
					<div class="row">
						<label class="label" for="a-unit">단위<span
							style="color: #ef4444"> *</span></label> <input class="input" type="text"
							name="unit" id="a-unit" placeholder="kg / L / 개" required />
					</div>
					<div class="row">
						<label class="label" for="a-unitPrice">단가</label> <input
							class="input" type="number" name="unit_price" id="a-unitPrice"
							step="0.01" min="0" />
					</div>
					<div class="row">
						<label class="label" for="a-step">주문단위(팩당 단위수)</label> <input
							class="input" type="number" name="step" id="a-step" step="0.01"
							min="0" />
					</div>

					<!-- 이미지 URL(텍스트) + 파일 -->
					<!-- <div class="row">
						<label class="label" for="a-imgPath">이미지 URL</label> <input
							class="input" type="text" name="img_path" id="a-imgPath"
							placeholder="/uploads/materials/xxx.jpg" />
					</div> -->
					<input type="file" name="image" id="a-image" accept="image/*"
						style="display: none" />
    ...
    function openEdit(tr){
      ...
      const imgFileName = tr.dataset.imgPath || ''; // DB에 저장된 파일명
      $('#f-prev').value = imgFileName; // 기존 파일명을 hidden input에 저장

      const previewImg = $('#f-preview');
      if (imgFileName) {
          // 파일명이 있으면 ImageServlet을 통해 미리보기 표시
          previewImg.src = '${contextPath}/images/' + encodeURIComponent(imgFileName);
      } else {
          previewImg.src = '${contextPath}/img/plus.png';
      }
