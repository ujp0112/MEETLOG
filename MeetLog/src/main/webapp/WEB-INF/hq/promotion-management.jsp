	...
	<main>
		<section class="panel" aria-labelledby="pageTitle">
			<div class="hd">
				...
				<a class="btn primary" href="${contextPath}/hq/promotion/form">+
					프로모션 추가</a>
			</div>
			<div class="bd">
				<!-- toolbar -->
					...
					<table class="sheet" role="grid">
						...
						<tbody>
							<c:forEach var="p" items="${promotions}">
								...
								<c:set var="status"
									value="${now.after(p.endDate) ? '종료' : (now.before(p.startDate) ? '예정' : '진행중')}" />
								<tr>
									<td><mytag:image fileName="${p.imgPath}"
											altText="${p.title}" cssClass="thumb" /></td>
									<td>${fn:escapeXml(p.title)}</td>
									<td><fmt:formatDate value="${p.startDate}"
											pattern="yyyy.MM.dd HH:mm" /> ~ <fmt:formatDate
											value="${p.endDate}" pattern="yyyy.MM.dd HH:mm" /></td>
									<td><span
										class="badge ${status eq '진행중' ? 'ok' : (status eq '예정' ? 'warn' : 'na')}">${status}</span>
									</td>
									<td>
										<div class="row-actions">
											<a href="${contextPath}/hq/promotion/form?id=${p.id}"
												class="btn-sm">수정</a>
